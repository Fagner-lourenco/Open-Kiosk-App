...
/**
 * Trigger: onCreate para orders no path legado
 * 
 * Path: stores/{storeId}/orders/{orderId}
 */
export const onLegacyOrderCreated = functions
  .region('southamerica-east1')
  .firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    console.log(`[aggOrders] Legacy onCreate: ${orderId} in store ${storeId}`);
    ...
    try {
      ...
      console.error(`[aggOrders] ❌ Error updating legacy metrics:`, error);
      throw error;
    }
  });

/**
 * Trigger: onUpdate para orders no path legado
 */
export const onLegacyOrderUpdated = functions
  .region('southamerica-east1')
  .firestore
  .document('stores/{storeId}/orders/{orderId}')
  .onUpdate(async (change, context) => {
    ...
    console.log(`[aggOrders] Legacy onUpdate: ${orderId}`);
