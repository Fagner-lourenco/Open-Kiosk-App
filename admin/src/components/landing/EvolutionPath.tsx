  ...
  return (
    <div className="grid gap-8 md:grid-cols-3 relative">
      ...
      {EVOLUTION_STEPS.map((step, index) => (
        <div
          key={step.number}
          className="relative bg-card border rounded-lg p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group"
        >
          {/* Número grande de fundo */}
          <div className="absolute -top-4 left-6 text-6xl font-bold text-primary/10 select-none">
            {step.number}
          </div>
